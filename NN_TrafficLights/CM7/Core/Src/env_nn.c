...
float proportions[4] = {1.0, 1.06, 1.03, 1.05}; // must be bigger than 0.98, varies about 1.0 to 1.6 {0.98, 0.99, 0.98574, 0.9854}
...
void event_routine(struct Environment * env){
	...
	if (env->current_sec == env->event_start){
		...
		assert_event(env);
		if (verbosity > 0){
			num_of_chars = sprintf(buffer, "EVENT START!\n");
			HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 20);
		}
	}
	if (env->event_en == 1){
		if (env->event_cnt == 0){
			env->event_en = 0;
			if (verbosity > 0){
				num_of_chars = sprintf(buffer, "EVENT END!\n");
				HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 20);
			}
			assert_event(env);
			env->event_scalar = 1.0;
...
void event_eod_routine(struct Environment * env){
	...
	uint32_t tempx;
	int temp;
	status = HAL_RNG_GenerateRandomNumber(&hrng, &tempx);
	if (status != HAL_OK){
		num_of_chars = sprintf(buffer, "RNG ERROR!\n");
		HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 20);
	}
	...
	if (temp > 500){
		...
		if (verbosity > 0){
			num_of_chars = sprintf(buffer, "ev_start = %d, ev_cnt = %d, scal = %f, lig = %d \n", env->event_start, env->event_cnt, env->event_scalar, env->event_lights);
...
void count_leaving(struct Environment * env, int verbosity){
	...
	int num_of_chars = 0;
	if (verbosity > 0){
		num_of_chars = sprintf(buffer, "env_nor : {%d, %d, %d, %d}\n", env->num_of_arrivals[0], env->num_of_arrivals[1], env->num_of_arrivals[2], env->num_of_arrivals[3]);
		HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 80);
		num_of_chars = sprintf(buffer, "env_arriv : {%d, %d, %d, %d}\n", env->arrived_in_last_cycle[0], env->arrived_in_last_cycle[1], env->arrived_in_last_cycle[2], env->arrived_in_last_cycle[3]);
		HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 80);
	}
	...
	if (verbosity > 0){
		num_of_chars = sprintf(buffer, "env_left : {%d, %d, %d, %d}\n", env->num_of_left[0], env->num_of_left[1], env->num_of_left[2], env->num_of_left[3]);
		HAL_UART_Transmit(&huart3, (uint8_t*)buffer, num_of_chars, 80);
	}
}
